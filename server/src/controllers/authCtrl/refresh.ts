...
const refresh = async (req: Request, res: Response) => {
  try {
    ...
    if (!decoded.id)
      return res
        .status(400)
        .json({ msg: "Sorry, your session has expired. Please sign out and sign in again to continue." });
    ...
    if (!user.rf_token) return res.status(400).json({ msg: "Please sign in first." });
